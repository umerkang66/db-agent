...
export class ReplSession {
  ...
  private printWelcomeBanner(): void {
    const banner = [
      ...
      `${chalk.bold('Strict Mode:')}   ${this.strictMode ? (this.allowFullWipe ? chalk.yellow('ON (Wipes & admin allowed)') : chalk.green('ON (Wipes & admin blocked)')) : chalk.yellow('OFF')}`,
      `${chalk.bold('Admin Tasks:')}   ${this.allowFullWipe ? chalk.green('ALLOWED (--allow-full-wipe)') : chalk.gray('BLOCKED (start with --allow-full-wipe)')}`,
